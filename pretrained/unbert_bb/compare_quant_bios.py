...
Usage:
    python compare_quant_bios.py
    python compare_quant_bios.py --sanity-check  # quick test with 200 samples
"""
...
def extract_hidden_states(model, loader, device, max_samples=1000):
    """Returns (13, N, 768) float32 numpy array ofCLS] embeddings per layer."""
    model.eval()
    all_hidden = []
    n_collected = 0
...
# ════════════════════════════════════════════════════════════════
# 7. PLOTS (continued in next message due to length)
# ════════════════════════════════════════════════════════════════
...
def main():
    ...
    if not os.path.exists(args.model_path):
        ...
        print("Please run 'train_bios.py' first.\n")
        return
    ...
    print("\n══════ REPRESENTATION ANALYSIS ══════")
    repr_loader = DataLoader(test_ds, batch_size=64, shuffle=False, num_workers=4)
